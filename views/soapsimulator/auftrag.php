<!DOCTYPE html>
<html lang="de">
<head>
    <meta charset="utf-8">
    <meta
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>S/PRI-Auftrag</title>
    <?php
        echo \Fuel\Core\Asset::css('bootstrap.css');
        echo \Fuel\Core\Asset::js('bootstrap.js');
    ?>
    <style>
        #logo{
            display: block;
            background-image: url(data:image/png;base64,);
            width: 179px;
            height: 45px;
            position: relative;
            top: 15px;
        }
        header{
            background-image: url(data:image/jpeg;base64,);
            height: 75px;
            width: 100%;
            margin-bottom: 40px;
        }
        footer{
            background-image: url(data:image/jpeg;base64,);
            width: 100%;
            margin-bottom: 40px;
        }
    </style>
<script>
    function initKWT()
    {
        let kwt = new Date(); // not instatiated with date!!! DANGER
        let newDate = new Date();
        newDate.setDate(kwt.getDate() + 14);
        console.log(newDate.getFullYear() + '-' + (newDate.getMonth()+1) + '-' + newDate.getDate());
        document.getElementById("KWT").value = newDate.getFullYear() + '-' + (newDate.getMonth()+1) + '-' + newDate.getDate();
    }

    function selectPartner(event)
    {
        const partner = [
            <?php
            foreach($arr_auftraggeber as $i =>$data)
            {
                echo '{ ';
                $cnt = count($data);
                foreach($data as $key => $value)
                {
                    echo $key.' : "'.$value.'"'; if($cnt > 1) echo ', ';
                    $cnt--;
                }
                echo '} '; if($i<count($arr_auftraggeber)-1) echo ', ';
            }
            ?>
        ];
        let id = event.target.value;
        let selected = partner[id];
        document.getElementById("itu_carrier_code").value = selected["icc"];
        document.getElementById("auftraggeber_nr").value = selected["agnr"];
        document.getElementById("serial").value = selected["serial"];
        document.getElementById("issuer").value = selected["issuer"];

    }
    document.addEventListener('DOMContentLoaded', (event) => {
        console.log("DOM fully loaded and parsed");
        const selectAuftraggeber = document.getElementById("Auftraggeber");
        selectAuftraggeber.addEventListener("change", (event) => {
            selectPartner(event);
        });
        initKWT();
    });

</script>
</head>
<body>
<header>
<div class="container">
<div id="logo"></div>
</div>
</header>
<main>
    <div class="container">
        <h1>SOAP-Orderer</h1>
        <div class="row col-md-12">
            <form id="AuftragForm" method="post" action="<?php echo \Fuel\Core\Router::get('soap_simulator_sende_auftrag_request'); ?>">
                <input type="hidden" class="form-control" id="itu_carrier_code" name="icc" value="">
                <input type="hidden" class="form-control" id="auftraggeber_nr" name="auftraggeber_nr" value="">
                <input type="hidden" class="form-control" id="serial" name="serial" value="">
                <input type="hidden" class="form-control" id="issuer" name="issuer" value="">
                <div class="card">

                    <div class="card-header">Auftragserstellung</div>
                        <div class="card-body">
                            <ul class="list-group">
                                <li class="list-group-item">
                                    <div class="card">
                                        <div class="card-header">Auftrag</div>
                                        <div class="card-body">
                                            <div class="form-group col-sm-10 row mb-2">
                                                <div class="col-sm-5">
                                                    <select class="form-select" id="AuftragsTyp" name="AuftragsTyp">
                                                <?php if($arr_Auftragstyp) {
                                                    foreach($arr_Auftragstyp as  $atyp_name) { ?>
                                                        <option value="<?php echo $atyp_name; ?>"><?php echo $atyp_name; ?></option>
                                                    <?php }
                                                } ?>
                                                    </select>
                                                </div>
                                            </div>
                                            <div class="form-group mb-2 row">
                                                <label for="ExterneAuftragsnummer" class="col-sm-2 col-form-label">ExterneAuftragsnummer</label>
                                                <div class="col-sm-10">
                                                    <input type="text" class="form-control" id="ExterneAuftragsnummer"  name="ExterneAuftragsnummer" value="">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </li>
                                <li class="list-group-item">
                                    <div class="card">
                                        <div class="card-header">Auftraggeber</div>
                                        <div class="card-body">
                                            <div class="form-group row">
                                                <label for="Auftraggeber" class="col-sm-2 col-form-label">Auftraggebernummer</label>
                                                <div class="col-sm-9 mb-2">
                                                    <select class="form-select" id="Auftraggeber" name="Auftraggeber">
                                                        <option value="-1">Auftraggeber wählen</option>
                                                        <?php if($arr_auftraggeber) {
                                                            foreach($arr_auftraggeber as $pid => $ag) { ?>
                                                                <option value="<?php echo $pid; ?>"><?php echo $ag["agnr"]." (".$ag["agname"].")"; ?></option>
                                                            <?php }
                                                        } ?>
                                                    </select>
                                                </div>

                                                <label for="Leistungsnummer" class="col-sm-2 col-form-label">Leistungsnummer</label>
                                                <div class="col-sm-6">
                                                    <input type="text" class="form-control" name="Leistungsnummer" id="Leistungsnummer" value="12345678">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </li>
                                <li class="list-group-item">
                                    <div class="card">
                                        <div class="card-header">Geschäftsfall</div>
                                        <div class="card-body">
                                            <div class="form-group row mb-2">
                                                <label for="Vertragsnummer" class="col-sm-2 col-form-label">Vertragsnummer</label>
                                                <div class="col-sm-10">
                                                    <input type="text" class="form-control" name="Vertragsnummer" id="Vertragsnummer" value="">
                                                </div>
                                            </div>
                                            <div class="form-group row mb-2">
                                                <label for="LineId" class="col-sm-2 col-form-label">LineId</label>
                                                <div class="col-sm-10">
                                                    <input type="text" class="form-control" name="LineId" id="LineId" value="">
                                                </div>
                                            </div>
                                            <div class="form-group row mb-2">
                                                <label for="VorabstimmungId" class="col-sm-2 col-form-label">VorabstimmungId</label>
                                                <div class="col-sm-10">
                                                    <input type="text" class="form-control" name="VorabstimmungId" id="VorabstimmungId" value="">
                                                </div>
                                            </div>
                                            <div class="card">
                                                <div class="card-header">Ansprechpartner</div>
                                                <div class="card-body">
                                                    <div class="card">
                                                        <div class="card-header">Auftragsmanagement</div>
                                                        <div class="card-body">
                                                            <div class="form-group row mb-2">
                                                                <label for="Anrede" class="col-sm-2 col-form-label">Anrede</label>
                                                                <div class="col-sm-10">
                                                                    <select class="form-select" id="Auftragsmanagement[Anrede]" name="Auftragsmanagement[Anrede]">
                                                                        <option value="9">keine Anrede</option>
                                                                        <option value="1">Herr</option>
                                                                        <option value="2">Frau</option>
                                                                        <option value="4">Firma</option>
                                                                    </select>
                                                                </div>
                                                                <label for="Auftragsmanagement[Vorname]" class="col-sm-2 col-form-label">Vorname</label>
                                                                <div class="col-sm-10">
                                                                    <input type="text" class="form-control" name="Auftragsmanagement[Vorname]" id="Auftragsmanagement[Vorname]" value="Betty M.">
                                                                </div>
                                                                <label for="Auftragsmanagement[Nachname]" class="col-sm-2 col-form-label">Nachname</label>
                                                                <div class="col-sm-10">
                                                                    <input type="text" class="form-control" name="Auftragsmanagement[Nachname]" id="Auftragsmanagement[Nachname]" value="Bocanegra">
                                                                </div>
                                                                <label for="Auftragsmanagement[Telefonnummer]" class="col-sm-2 col-form-label">Telefonnummer</label>
                                                                <div class="col-sm-10">
                                                                    <input type="text" class="form-control" name="Auftragsmanagement[Telefonnummer]" id="Auftragsmanagement[Telefonnummer]" value="04515559222">
                                                                </div>
                                                                <label for="Auftragsmanagement[Emailadresse]" class="col-sm-2 col-form-label">Emailadresse</label>
                                                                <div class="col-sm-10">
                                                                    <input type="text" class="form-control" name="Auftragsmanagement[Emailadresse]" id="Auftragsmanagement[Emailadresse]" value="auftrags@management.net">
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="card">
                                                <div class="card-header">Termine</div>
                                                <div class="card-body">
                                                    <div class="card">
                                                        <div class="card-header">AuftraggeberWunschtermin </div>
                                                        <div class="card-body">
                                                            <div class="form-group row col-sm-12 mb-2">
                                                                <label for="AuftraggeberWunschtermin[Datum]" class="col-sm-2 col-form-label">Datum</label>
                                                                <div class="col-sm-3">
                                                                    <input type="date" class="form-control" name="AuftraggeberWunschtermin[Datum]" id="KWT" value="">
                                                                </div>
                                                            </div>
                                                            <div class="form-group row col-sm-12 mb-2">
                                                                <label for="AuftraggeberWunschtermin[Zeitfenster]" class="col-sm-2 col-form-label">Zeitfenster</label>
                                                                <div class="col-sm-3">
                                                                    <input type="text" class="form-control" name="AuftraggeberWunschtermin[Zeitfenster]" id="AuftraggeberWunschtermin[Zeitfenster]" value="26">
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="card">
                                                <div class="card-header">Auftragsposition </div>
                                                <div class="card-body">
                                                    <div class="card">
                                                        <div class="card-header">Produkt</div>
                                                        <div class="card-body">
                                                            <div class="form-group row mb-2">
                                                                <label for="Produkt[Bezeichner]" class="col-sm-2 col-form-label">Bezeichner</label>
                                                                <div class="col-sm-10">
                                                                    <input type="text" class="form-control" name="Produkt[Bezeichner]" id="Produkt[Bezeichner]" value="FTTH L2 XGSPON 300 150">
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="card">
                                                        <div class="card-header">GeschaeftsfallProdukt</div>
                                                        <div class="card-body">
                                                            <div class="card">
                                                                <div class="card-header">StandortA</div>
                                                                <div class="card-body">
                                                                    <div class="card">
                                                                        <div class="card-header">Person</div>
                                                                        <div class="card-body">
                                                                            <div class="form-group row mb-2">
                                                                                <label for="StandortA[Anrede]" class="col-sm-2 col-form-label">Anrede</label>
                                                                                <div class="col-sm-10">
                                                                                    <select class="form-select" id="StandortA[Anrede]" name="StandortA[Anrede]">
                                                                                        <option value="9">keine Anrede</option>
                                                                                        <option value="1">Herr</option>
                                                                                        <option value="2">Frau</option>
                                                                                        <option value="4">Firma</option>
                                                                                    </select>
                                                                                </div>
                                                                            </div>
                                                                            <div class="form-group row mb-2">
                                                                                <label for="StandortA[Vorname]" class="col-sm-2 col-form-label">Vorname</label>
                                                                                <div class="col-sm-10">
                                                                                    <input type="text" class="form-control" name="StandortA[Vorname]" id="StandortA[Vorname]" value="Gabriele">
                                                                                </div>
                                                                            </div>
                                                                            <div class="form-group row mb-2">
                                                                                <label for="StandortA[Nachname]" class="col-sm-2 col-form-label">Nachname</label>
                                                                                <div class="col-sm-10">
                                                                                    <input type="text" class="form-control" name="StandortA[Nachname]" id="StandortA[Nachname]" value="Nussbaum">
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                    <div class="card">
                                                                        <div class="card-header">Strasse</div>
                                                                        <div class="card-body">
                                                                            <div class="form-group row mb-2">
                                                                                <label for="StandortA[Strassenname]" class="col-sm-2 col-form-label">Strassenname</label>
                                                                                <div class="col-sm-10">
                                                                                    <input type="text" class="form-control" name="StandortA[Strassenname]" id="StandortA[Strassenname]" value="Georgstraße">
                                                                                </div>
                                                                            </div>
                                                                            <div class="form-group row mb-2">
                                                                                <label for="StandortA[Hausnummer]" class="col-sm-2 col-form-label">Hausnummer</label>
                                                                                <div class="col-sm-10">
                                                                                    <input type="text" class="form-control" name="StandortA[Hausnummer]" id="StandortA[Hausnummer]" value="28">
                                                                                </div>
                                                                            </div>
                                                                            <div class="form-group row mb-2">
                                                                                <label for="StandortA[HausnummernZusatz]" class="col-sm-2 col-form-label">HausnummernZusatz</label>
                                                                                <div class="col-sm-10">
                                                                                    <input type="text" class="form-control" name="StandortA[HausnummernZusatz]" id="StandortA[HausnummernZusatz]" value="A">
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                    <div class="card">
                                                                        <div class="card-header">Ort</div>
                                                                        <div class="card-body">
                                                                            <div class="form-group row mb-2">
                                                                                <label for="StandortA[PLZ]" class="col-sm-2 col-form-label">PLZ</label>
                                                                                <div class="col-sm-10">
                                                                                    <input type="text" class="form-control" name="StandortA[PLZ]" id="StandortA[PLZ]" value="23560">
                                                                                </div>
                                                                            </div>
                                                                            <div class="form-group row mb-2">
                                                                                <label for="StandortA[Ortsname]" class="col-sm-2 col-form-label">Ortsname</label>
                                                                                <div class="col-sm-10">
                                                                                    <input type="text" class="form-control" name="StandortA[Ortsname]" id="StandortA[Ortsname]" value="Lübeck">
                                                                                </div>
                                                                            </div>
                                                                            <div class="form-group row mb-2">
                                                                                <label for="StandortA[Land]" class="col-sm-2 col-form-label">Land</label>
                                                                                <div class="col-sm-10">
                                                                                    <input type="text" class="form-control" name="StandortA[Land]" id="StandortA[Land]" value="de">
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class="card">
                                                                <div class="card-header">Vormieter</div>
                                                                <div class="card-body">
                                                                    <div class="form-group row mb-2">
                                                                        <label for="Vormieter[Nachname]" class="col-sm-2 col-form-label">Nachname</label>
                                                                        <div class="col-sm-10">
                                                                            <input type="text" class="form-control" name="Vormieter[Nachname]" id="Vormieter[Nachname]" value="Chubb-Baggins">
                                                                        </div>
                                                                    </div>
                                                                    <div class="form-group row mb-2">
                                                                        <label for="Vormieter[HomeIdNummer]" class="col-sm-2 col-form-label">HomeIdNummer</label>
                                                                        <div class="col-sm-10">
                                                                            <input type="text" class="form-control" name="Vormieter[HomeIdNummer]" id="Vormieter[HomeIdNummer]" value="">
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class="card">
                                                                <div class="card-header">Technologie</div>
                                                                <div class="card-body">
                                                                    <div class="form-group row mb-2">
                                                                        <label for="Technologie" class="col-sm-2 col-form-label">Technologie</label>
                                                                        <div class="col-sm-10">
                                                                            <input type="text" class="form-control" name="Technologie" id="Technologie" value="FTTH">
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class="card">
                                                                <div class="card-header">Parameter</div>
                                                                <div class="card-body">
                                                                    <div class="form-group row mb-2">
                                                                        <label for="Parameter[Operation]" class="col-sm-2 col-form-label">Operation</label>
                                                                        <div class="col-sm-10">
                                                                            <input type="text" class="form-control" name="Parameter[Operation]" id="Parameter[Operation]" value="registrierenONT">
                                                                        </div>
                                                                    </div>
                                                                    <div class="form-group row mb-2">
                                                                        <label for="Parameter[Operation]" class="col-sm-2 col-form-label">Operation</label>
                                                                        <div class="col-sm-10">
                                                                            <input type="text" class="form-control" name="Parameter[Operation]" id="Parameter[Operation]" value="PM:AVM1418949498">
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>

                                                </div>
                                            </div>

                                        </div>
                                    </div>
                                </li>
                                <li class="list-group-item">
                                    <div class="form-group mb-2 row">
                                        <label for="GeschaeftsfallArt" class="col-sm-2 col-form-label">GeschaeftsfallArt</label>
                                        <div class="col-sm-4">
                                            <select class="form-select" id="GeschaeftsfallArt" name="GeschaeftsfallArt">
                                                <?php if($arrGeschaeftsfallArten) {
                                                    foreach($arrGeschaeftsfallArten as $gfArt) { ?>
                                                        <option value="<?php echo $gfArt; ?>"><?php echo $gfArt; ?></option>
                                                    <?php }
                                                } ?>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="form-group mb-2 row">
                                        <label for="Aenderungskennzeichen" class="col-sm-2 col-form-label">Aenderungskennzeichen</label>
                                        <div class="col-sm-3">
                                            <select class="form-select" id="Aenderungskennzeichen" name="Aenderungskennzeichen">
                                                <?php if($arrAenderungsKennzeichen) {
                                                    foreach($arrAenderungsKennzeichen as $akz) { ?>
                                                        <option value="<?php echo $akz; ?>"><?php echo $akz; ?></option>
                                                    <?php }
                                                } ?>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="form-group mb-2 row">
                                        <label for="TerminNeu" class="col-sm-2 col-form-label">TerminNeu</label>
                                        <div class="col-sm-2">
                                            <input type="date" class="form-control" name="TerminNeu" id="TerminNeu" value="">
                                        </div>
                                    </div>
                                </li>
                                <li class="list-group-item">
                                    <button type="submit" class="btn btn-primary">Auftrag senden</button>
                                </li>
                            </ul>
                        </div>
                    </div>

                </form>
            </div>
        </div>
    </main>
    <footer>
        <div class="row">
            <div class="col-md-6 offset-md-5">
                <p class="text-end text-light">Page rendered in {exec_time}s using {mem_usage}mb of memory.</p>
                <p class="text-end text-light">Fuel-Enviroment: <?php echo Fuel::$env; ?></p>
                <p class="text-end text-light">
                    <a href="https://fuelphp.com">FuelPHP</a> is released under the MIT license.<br>
                    <small>Version: <?php echo Fuel::VERSION; ?></small>
                </p>
            </div>
        </div>

    </footer>
</body>
</html>
